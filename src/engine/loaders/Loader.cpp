...
namespace Engine
{
    namespace Loaders
    {
        ...
        RawModelPtr Loader::Load(std::vector<GLfloat> vertex_buffer, std::vector<GLfloat> textcoord_buffer) {
            ...
            glBufferData(GL_ARRAY_BUFFER, textcoord_buffer.size() * sizeof(GLfloat), textcoord_buffer.data(),
                         GL_STATIC_DRAW);
        ...
        void Loader::LoadVertexBuffer(GLuint location, std::vector<GLfloat> vertex_buffer) {
            ...
            glBufferData(GL_ARRAY_BUFFER, vertex_buffer.size() * sizeof(GLfloat), vertex_buffer.data(),
                         GL_STATIC_DRAW);
        }
    }
}
