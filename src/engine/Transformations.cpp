    ...
    BitMapPtr Transformations::Scale(BitMapPtr bitmap, int new_width, int new_height) {
        ...
        for (int i = 0; i < new_height; i++) {
            for (int j = 0; j < new_width; j++) {
                ...
                (*scaled->buffer)[i * new_width + j] = {(unsigned char) red, (unsigned char) green,
    ...
    BitMapPtr Transformations::Center(BitMapPtr background, BitMapPtr foreground) {
        ...
        for (int y = 0; y < foreground->height; y++) {
            for (int x = 0; x < foreground->width; x++) {
                (*background->buffer)[top_margin + y * foreground->width + x + left_margin +
                                      offset] = (*foreground->buffer)[y * foreground->width + x];
            }
