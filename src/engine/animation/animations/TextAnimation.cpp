            ...
            AnimationStatus TextAnimation::Animate(float delta) {
                ...
                if (status == AnimationStatus::Skipped)
                    ...
                    if (status == AnimationStatus::Default || status == AnimationStatus::MovedToNext) {
                        ...
                        TextKeyFramePtr nextKeyFrame = std::static_pointer_cast<TextKeyFrame>(
                                keyFrames[currentKeyFrameIndex + 1]);

                        float sourceTextLength = (float) std::static_pointer_cast<TextKeyFrame>(
                                keyFrames[currentKeyFrameIndex])->text.length();
